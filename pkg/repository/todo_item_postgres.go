...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti 
					...
					INNER JOIN %s ul on ul.lists_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2`,
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todo.TodoItem, error) {
	...
	query := fmt.Sprintf(`SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti 
								INNER JOIN %s li on li.item_id = ti.id
					INNER JOIN %s ul on ul.lists_id = li.list_id WHERE ti.id = $1 AND ul.user_id = $2`,
		todoItemsTable, listsItemTable, usersListsTable)
...
func (r *TodoItemPostgres) Delete(userId, itemId int) error {
	query := fmt.Sprintf(`DELETE FROM %s ti USING %s li, %s ul
						WHERE ti.id = li.item_id AND li.list_id = ul.lists_id AND ul.user_id = $1 AND ti.id = $2`,
		todoItemsTable, listsItemTable, usersListsTable)
...
func (r *TodoItemPostgres) Update(userId, itemId int, input todo.UpdateItemInput) error {
	...
	query := fmt.Sprintf(`UPDATE %s ti SET %s FROM %s li, %s ul WHERE ti.id = li.item_id 
								AND li.list_id = ul.lists_id And ul.user_id = $%d AND ti.id = $%d`,
